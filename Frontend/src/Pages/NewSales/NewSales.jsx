  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (result.status) {
        ...
        toast.error("Unable to create dispatch record");
        ...
        <div className="drawer-content px-4">
          ...
          <div className=''>
            ...
            <div className="flex items-center justify-between my-4">
              ...
              <h2 className='text-xl w-full text-center'>New Dispatch</h2>
            </div>

            <form onSubmit={handleSubmit(onSubmit)} id='form_new_sales'>
              <h4 className='text-xl capitalize font-bold'>Client Details </h4>
              <div className="form-box flex flex-wrap md:flex-nowrap">
                <div className='w-full'>
                  <label className="form-control lg:max-w-xs px-2">
                    ...
                    <input type="text"
                      ...
                      placeholder="Enter client name" className="input input-bordered w-full lg:max-w-xs " />
                  </label>
                ...
                <div className="w-full">
                  <label className="form-control w-full lg:max-w-xs px-2">
                    ...
                    <input type="email"
                      ...
                      placeholder="Enter client email" className="input input-bordered w-full lg:max-w-xs " />
                  </label>
                  <div className="label-text text-xs text-error h-8 p-2">
                ...
                <div className="w-full">
                  <label className="form-control w-full lg:max-w-xs px-2">
                    <div className="label">
                      <span className="label-text">Client Contact</span>
                    </div>
                    <input type="number"
                      name='cust_contact'
                      {...register('cust_contact', { validate: validateCustContact })}
                      placeholder="Enter client contact" className="input input-bordered w-full lg:max-w-xs " />
                  </label>
                  <div className="label-text text-xs text-error h-8 p-2">
              ...
              {/* order details */}
              <h4 className='text-xl mt-3  font-bold'>Dispatch Details </h4>
              <div className="form-box flex items-center flex-wrap border-b-2 pb-2">
                ...
                <label className="form-control  px-2">
                  <div className="label">
                    <span className="label-text">Dispatch Quantity (L)</span>
                  </div>
                  <input type="number"
                ...
                <label className="form-control px-2">
                  <div className="label">
                    <span className="label-text">Dispatch Discount (%)</span>
                  </div>
                  <input type="number"
                ...
                <label className="form-control w-24">
                  <div className="label">
                    <span className="label-text">Dispatch Value</span>
                  </div>
                  <input type="text" placeholder="Auto Calculated" value={selectedProductDetails.subtotal} className="input input-bordered w-24 " disabled />
                </label>
                <div className="text-center p-2">
                  <p className='pt-2 hidden lg:block'>Add Batch</p>
                  <button onClick={handleAddToCart} className="btn btn-primary ">Add Batch</button>
                </div>
              </div>
              {/* </form> */}
                      ...
                      {cartItems.map((elem, i, arr) => {
                        return (
                          <tr className="hover" key={i}>
                            ...
                            <td>₹{elem.c_subtotal}</td>
                          </tr>
                        )
                      })}
                      <tr className="hover">
                        <td colSpan={4}></td>
                        <td>Total Dispatch Value</td>
                        <td>₹{cartItems.reduce((acc, obj) => { return (parseFloat(acc) + parseFloat(obj.c_subtotal)).toFixed(2) }, 0)}</td>
                      </tr>
                      <tr>
                        <td colSpan={6} className='text-center'>
                          <button type='submit' className='btn btn-primary'>Create Dispatch Record</button></td>
                      </tr>
                    </tbody>
                  </table>
                </div>}
