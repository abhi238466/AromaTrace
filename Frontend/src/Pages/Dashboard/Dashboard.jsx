...
const Dashboard = () => {
    ...
    return (isLogin &&
        <div className='md:w-[80%] md:mx-auto'>
            ...
            <div className="drawer lg:drawer-open">
                ...
                <div className="drawer-content px-4">
                    <div className="flex items-center justify-between ">
                        ...
                        <h2 className='text-xl'>Production Batches</h2>
                        {/* search bar */}
                        <form action="" className='hidden md:flex items-center w-1/2'>
                            <input type="text" placeholder="Search Batch" className="input input-bordered rounded-full h-10 lg:w-full" />
                    ...
                    <form action="" className='flex md:hidden items-center w-full mt-4'>
                        <input type="text" placeholder="Search Batch" className="input input-bordered rounded-full h-10 w-full" />
                        <button type="submit" className='serch p-2 bg-blue-500 text-white rounded-md ms-2'>
                            <MagnifyingGlassIcon className='w-6 h-6' />
                        </button>
                    </form>
                    ...
                    {isFetchFinished && products.length <= 0 && <div className='text-sm px-2 text-center'>No Batch Records Found!<br/>Click + to Create Your First Batch</div>}
                    {/* table end */}
                </div>
            ...
            {/* modal */}
            <ModalAdd id="add_modal" title="Create New Batch" fetchProducts={fetchProducts} />
            <ModalDelete id="delete_modal" pid={pid} title="Delete Batch Record" fetchProducts={fetchProducts}/>
            <ModalUpdate id="update_modal" title="Update Batch Details" updateObj={updateObj} fetchProducts={fetchProducts} />
        </div>
    )
}
